...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.js')){event.respondWith(fetch(event.request,{cache:'no-store'}).then(async r=>{const text=await r.text();const extra=await fetch('./v7.js?v=7',{cache:'no-store'}).then(x=>x.text());const v8=await fetch('./v8.js?v=8',{cache:'no-store'}).then(x=>x.text());return new Response(text+'\n;'+extra+'\n;'+v8,{headers:{'Content-Type':'application/javascript; charset=utf-8'}})}));return;}
